...
def InitialState(rf_obs: RecvFunc, swd_obs: SurfaceWaveDisp, lims: Limits, random_seed: int) -> State:
    ...
    return State(model_0, cov_m0, rf_synth_m0, swd_synth_m0, fit_to_obs_m0)


def Iterate(rf_obs: RecvFunc, swd_obs: SurfaceWaveDisp, lims: Limits,
            itr: int, current_state: State) -> (State, bool):
    if not itr % 5:
        print("Iteration {}..".format(itr))

    # Generate new model by perturbing the old model
    model, changes_model = Mutate(current_state.model,itr)

    if not CheckPrior(model, lims): # check if mutated model compatible with prior distr.
        print('Failed Prior')
        return current_state, False  # if not, continue to next iteration
    ...
    if changes_model.which_change == 'Noise':
        ...
        cov = CalcCovarianceMatrix(model,rf_obs,swd_obs)
        rf_synth = current_state.rf_synth
        swd_synth = current_state.swd_synth
    else:
        cov = current_state.cov
        rf_synth = SynthesiseRF(fullmodel) # forward model the RF data
        swd_synth = SynthesiseSWD(fullmodel, swd_obs.period) # forward model the SWD data

    # Calculate fit
    fit_to_obs = Mahalanobis(
            rf_obs.amp, rf_synth.amp, # RecvFunc
            swd_obs.c, swd_synth.c,  # SurfaceWaveDisp
            cov.invCovar,             # CovarianceMatrix
            )

    # Calculate probability of keeping mutation
    (keep_yn, alpha) = AcceptFromLikelihood(
            fit_to_obs, current_state.fit_to_obs, # float
            cov, current_state.cov, # CovarianceMatrix
            changes_model, # form depends on which change
            )
    #print([changes_model.which_change, all_keep[itr-1], round(fit_to_obs_m,0)])
    if not keep_yn: # should be aiming to keep about 40% of perturbations
        return current_state, False
    return State(model, cov, rf_synth, swd_synth, fit_to_obs), True
